...
/*****   Notre classe qui masque le code C   *****/

using namespace std;

unsigned int PngLoader::load(string path)
{
	static map<string, unsigned int> _textures;
	map<string, unsigned int>::iterator i = _textures.find(path);
	unsigned int id = 0;
	if (i == _textures.end()) {
		id = loadPNGTexture(path.c_str());
		_textures[path] = id;
		return id;
	}
